...
def aug_code(dir: str,
    ...
    for idx in range(len(images)):
        ...
        # AUGMENTATION METHOD
        method = ['rddc', 'rdc', 'rldc', 'rbc', 'rlc', 'rn', 'rgn', 'fc', 'fs', 'ts', 'crr']
        ...
        for me in method:
            ...
            # SAVE IMAGE FILE
            new_image_file = new_dir + me + annotations[idx][2]
            cv2.imwrite(new_image_file, image_aug)

            logger.info(f'IMAGE FILE SUCCSSFULLY COMPLETED : {new_image_file}')

            # SAVE XML FILE
            h, w = np.shape(image_aug)[0:2]

            root = ET.Element("annotation")
            ET.SubElement(root, "filename").text = me + annotations[idx][2]
            ...
            new_xml_file = new_dir + me + annotations[idx][1]
            with open(new_xml_file, "w", encoding='utf-8') as f:
                f.write(xml_str)

            logger.info(f'XML FILE SUCCSSFULLY COMPLETED : {new_xml_file}')

            # SAVE TXT FILE
            classes = ['구진', '농포', '결절', '낭포', '결절/낭포', '켈로이드', '화이트헤드', '블랙헤드', '모낭염', '여드름자국', '여드름흉터', '표피낭종']

            basename = os.path.basename(new_xml_file)
            ...
            logger.info(f'TXT FILE SUCCSSFULLY COMPLETED : {new_dir}{filename}.txt')
